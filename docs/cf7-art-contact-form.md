...
<div class="contact__field contact__field--required">
	...
	[textarea* your-message class:contact__textarea maxlength:1000 placeholder "在庫の状況を教えてください。"]
...
<div class="contact-confirm__item">
	<p class="contact-confirm__label">お問い合わせ作品</p>
	<p class="contact-confirm__value" id="js-confirm-artwork"></p>
</div>
...
### 題名
```
【LURF GALLERY】作品お問い合わせ
```

### メッセージ本文
```
作品お問い合わせ: [your-artwork-info]
...
---
このメールはLURF GALLERYの作品お問い合わせフォームから送信されました。
```
...
作品お問い合わせいただきありがとうございます。
以下の内容で承りました。

---

お問い合わせ作品: [your-artwork-info]
メッセージ:
[your-message]
...
- `[hidden your-artwork-info]` にJS側から作品情報テキストをセット（PHPからURLパラメータ経由で取得した値）
- `wpcf7mailsent` イベントでサンクスページ `/art_contact/thanks/` へリダイレクト（`ref` パラメータ付き）
- `wpcf7invalid` イベントで確認画面→入力画面に戻す
- 固定ページのACFフィールド `formid` にCF7のフォームIDを設定
- バリデーションは `contact__field--required` クラスを持つフィールドを対象にJS側で実施
- 通常のcontactフォームとの違い: 「お問い合わせ内容」フィールドなし、作品情報セクションあり
- URLパラメータ: `artwork_image`, `artwork_artist`, `artwork_title`, `artwork_year`, `artwork_material`, `artwork_size`, `ref`
