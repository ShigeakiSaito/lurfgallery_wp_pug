<?php get_header(); ?>
	...
	<main class="single artwork-detail">
		...
		<div class="artwork-detail__container">
			<div class="artwork-detail__content">
				...
				<div class="artwork-detail__gallery">
					<div class="swiper js-artwork-swiper" id="artworkSwiper">
					...
					<div class="artwork-detail__nav">
						<button class="artwork-detail__prev js-artwork-prev" aria-label="前の画像"></button>
						<span class="artwork-detail__counter">
							<span class="js-artwork-current">1</span>
							<span class="artwork-detail__separator">/</span>
							<span class="js-artwork-total"><?php echo count($images); ?></span>
						</span>
						<button class="artwork-detail__next js-artwork-next" aria-label="次の画像"></button>
					</div>
				</div>
				<?php endif; ?>
				...
				<div class="artwork-detail__info">
					...
					<h2 class="artwork-detail__artist"><?php echo esc_html($artist_name['value']); ?></h2>
					<?php endif; ?>

					<?php $title = get_field('title'); ?>
					<?php if (!empty($title['is_display_detail']) && !empty($title['value'])) : ?>
					<h3 class="artwork-detail__title"><?php echo esc_html($title['value']); ?></h3>
					<?php endif; ?>

					<div class="artwork-detail__spec">
						<div class="artwork-detail__spec-items">
							<?php
							$spec_fields = [
								'year',
								'material',
						...
						<p class="artwork-detail__description">
					...
					?>
					<div class="artwork-detail__buttons">
						<?php if (!empty($buttons['contact'])) : ?>
						<a href="<?php echo esc_url($art_contact_url); ?>" class="artwork-detail__button">CONTACT</a>
						<?php endif; ?>
						<?php if (!empty($buttons['purchase'])) : ?>
						<a href="<?php echo esc_url(get_field('purchase_url')); ?>" class="artwork-detail__button">PURCHASE</a>
						<?php endif; ?>
					</div>
					<?php endif; ?>
				</div>
			...
			<!-- 戻るリンク -->
			<div class="artwork-detail__back">
				<a href="/exhibitions/" class="artwork-detail__back-link">
	...
	<script>
		window.addEventListener('load', function() {
			var swiperEl = document.querySelector('.js-artwork-swiper');
			if (!swiperEl) return;

			var slides = swiperEl.querySelectorAll('.swiper-slide');
			var totalEl = document.querySelector('.js-artwork-total');
			var currentEl = document.querySelector('.js-artwork-current');
			var navEl = document.querySelector('.artwork-detail__nav');
			...
			var artworkSwiper = new Swiper('#artworkSwiper', {
				loop: true,
				speed: 600,
				navigation: {
					prevEl: '.js-artwork-prev',
					nextEl: '.js-artwork-next',
				},
				on: {
